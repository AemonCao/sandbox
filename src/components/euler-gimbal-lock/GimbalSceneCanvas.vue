...
import { createGimbalScene } from '~/pages/euler-gimbal-lock/composables/useGimbalScenes'
...
let scene: ReturnType<typeof createGimbalScene> | null = null

function initScene() {
  if (!canvasRef.value)
    return

  scene = createGimbalScene(canvasRef.value)
  scene.init()
